...
const toggleSex = (sex) => {
  ...
  // 发送性别消息
  if (sex === 1) {
    sendMessage('期望AI是男生', 'userSex');
  } else if (sex === 2) {
    sendMessage('期望AI是女生', 'userSex');
  }
};
...
const handleCharacterSelect = (characterIndex) => {
  currentCharacter.value = characterIndex;
  const message = characterIndex === 0 ? '成熟稳重' : '幽默风趣';
  sendMessage(message, 'aiCharacter');
};
    ...
    .send_item_group {
      ...
      .left {
        ...
        .left_response_container {
          ...
          .active {
            color: #642727;
            background: linear-gradient(180deg, #FFDAAD 0%, #FF9A6B 100%);
          }
